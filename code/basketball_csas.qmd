...
2. How does league-wide FG% vary by quarter?
...
# period_stats is indexed by PLAYER_NAME; filter to matching players
top_period_stats = period_stats.loc[period_stats.index.isin(top_5_players)]
...
def plot_and_save_halfcourt(df, player, side='right', grid=220, bw_method=0.3, out_path=Path('./outputs')):
    ...
    try:
        ...
        return None
    # Support both 'playerName' and 'PLAYER_NAME' column names
    name_col = 'playerName' if 'playerName' in df2.columns else 'PLAYER_NAME'
    d = df2[df2[name_col] == player].dropna(subset=['x','y']).copy()
    if d.empty:
    ...
    html_path = out_path / f'{player}_halfcourt_heatmap.html'
    png_path = out_path / f'{player}_halfcourt_heatmap.png'
...
```{python}
dist_col = 'shotDistance' if 'shotDistance' in df.columns else ('SHOT_DISTANCE' if 'SHOT_DISTANCE' in df.columns else None)
if dist_col is not None:
    fig_dist = px.histogram(df.dropna(subset=[dist_col]), x=dist_col, nbins=40, title='Shot Distance Distribution', marginal='box')
...
```{python}
sample_player = top_scorers.iloc[0]['PLAYER']
name_col = 'playerName' if 'playerName' in df.columns else 'PLAYER_NAME'
result_col = 'shotResult' if 'shotResult' in df.columns else ('SHOT_MADE_FLAG' if 'SHOT_MADE_FLAG' in df.columns else None)
if 'YEAR' in df.columns and result_col is not None:
    dply = df[df[name_col]==sample_player].dropna(subset=[result_col])
    if result_col == 'SHOT_MADE_FLAG':
        yearly = dply.groupby('YEAR').agg(shots=(result_col,'count'), made=(result_col,'sum')).reset_index()
    else:
        yearly = dply.groupby('YEAR').agg(shots=(result_col,'count'), made=(result_col, lambda s: (s=='Made').sum())).reset_index()
    if not yearly.empty:
        yearly['FG%'] = yearly['made']/yearly['shots']
        fig_year = go.Figure()
        fig_year.add_trace(go.Bar(x=yearly['YEAR'], y=yearly['shots'], name='Shots'))
        fig_year.add_trace(go.Scatter(x=yearly['YEAR'], y=yearly['FG%'], name='FG%', yaxis='y2', mode='lines+markers'))
        fig_year.update_layout(title=f'{sample_player} - Shots and FG% per Year', template='plotly_dark', yaxis=dict(title='Shots'), yaxis2=dict(title='FG%', overlaying='y', side='right', tickformat='.0%'))
        fig_year.show()
